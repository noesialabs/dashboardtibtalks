...
function HeatmapGrid({ data, cellSize = 11 }: { data: HeatmapCell[]; cellSize?: number }) {
  ...
  return (
    <div className="relative overflow-auto">
      ...
      <div className="flex ml-10 mb-1">
        {Array.from({ length: 24 }, (_, i) => `${i}h`).filter((_, i) => i % 3 === 0).map((label, i) => (
          <div key={i} className="text-[9px] text-white/30" style={{ width: `${cellSize * 3 + 6}px` }}>
            {label}
          </div>
        ))}
      </div>
            ...
            <div className="flex gap-[2px]">
              {Array.from({ length: 24 }, (_, hour) => {
                ...
                return (
                  <div
                    key={hour}
                    className="rounded-[2px] cursor-pointer transition-all hover:scale-150 hover:z-10"
                    style={{
                      width: `${cellSize}px`,
                      height: `${cellSize}px`,
      ...
      {tooltip && (
        ...
        >
          <span className="text-white/60">{dayLabels[tooltip.day]} {tooltip.hour}h</span>
        ...
        <div className="flex gap-[2px]">
          {[0.1, 0.3, 0.5, 0.7, 0.9].map((opacity) => (
            <div
              ...
              style={{ width: `${cellSize}px`, height: `${cellSize}px`, backgroundColor: `rgba(139, 92, 246, ${opacity})` }}
            />
          ))}
        </div>
...
export function EngagementHeatmap({ data }: Props) {
  return (
    <ExpandableChart
      ...
      className="h-full"
      expandedContent={<HeatmapGrid data={data} cellSize={24} />}
    >
      <HeatmapGrid data={data} cellSize={11} />
    </ExpandableChart>
  );
}
